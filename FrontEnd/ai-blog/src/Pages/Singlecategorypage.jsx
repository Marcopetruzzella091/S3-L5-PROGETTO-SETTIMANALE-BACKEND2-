...
export default function Singlecategorypage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
          try {
            const cat = await axios.get('http://localhost/S3-L5-PROGETTO-SETTIMANALE-BACKEND2-/Backend/wp-json/wp/v2/posts?categories='+id);
            setcat(cat.data);
                ...
                cat.map(category => (
                    <Card className='my-3 catcardit' key={category.id} style={{ width: '18rem' }}>
                        ...
                        <Card.Body>
                            ...
                            <Button variant="primary" onClick={() => navigate(`/article/${category.id}`)}>Leggi tutto</Button>
                        </Card.Body>
                    </Card>
                ))
            )}
