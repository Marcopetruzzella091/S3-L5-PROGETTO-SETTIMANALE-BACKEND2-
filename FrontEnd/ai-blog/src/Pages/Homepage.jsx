...
export default function Homepage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const Documentazione = await axios.get('http://localhost/S3-L5-PROGETTO-SETTIMANALE-BACKEND2-/Backend/wp-json/wp/v2/');
  ...
  return (
    <>
      ...
      <Container>
        ...
        {loading ? <div className='mx-auto text-center'> <Spinner  animation="grow" variant="success" /></div> : <div className='row'>
          {data.map((post) => {
            ...
            return (
              <Card className='col-md-4 m-3 p-2' key={post.id} style={{ width: '18rem' }}>
                ...
                <Card.Body>
                  <Card.Title><h4 className=" font-size-2" dangerouslySetInnerHTML={{ __html: post.title.rendered }} /></Card.Title>
                  <Card.Text> 
                  <p className=" font-size-2" dangerouslySetInnerHTML={{ __html: post.excerpt.rendered }} />
                    
                  </Card.Text>
               {   <Button variant="primary"  onClick={() => navigate("/article/" +  post.id) }>Leggi tutto</Button> } 
